        ...
        <div className="p-4 md:p-6 max-w-7xl mx-auto">
            ...
            {getDisplayedInquiries().slice(0, 15).map((inquiry) => (
                <tr key={inquiry.id} className="hover:bg-secondary-700/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm flex gap-1 items-center">
                        {/* Only Show Actions if NOT Draft */}

                        {/* 1. Request Approval (If Draft) */}
                        {inquiry.quote_status === 'Draft' && (
                            <button onClick={() => handleRequestApproval(inquiry.id)} className="text-yellow-400 hover:text-yellow-300 border border-yellow-600 px-2 py-0.5 rounded text-xs" title="Request Approval">✋ Request</button>
                        )}

                        {/* 2. Edit (Always) */}
                        <button onClick={() => onEditInquiry && onEditInquiry(inquiry)} className="text-gray-400 hover:text-white" title="Edit">✏️</button>

                        {/* 3. Print Quote (If Approved) */}
                        <button onClick={() => onQuote && onQuote(inquiry)} className="text-blue-400 hover:text-blue-300" title="Print Quote">📄</button>

                        {/* 4. Print AWB (If AWB Exists) - NEW */}
                        {inquiry.awb_number && (
                            <button
